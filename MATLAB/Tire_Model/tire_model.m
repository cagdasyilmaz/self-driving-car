...
a1_x = -21.3; a2_x = 1144; a3_x = 49.6; a4_x = 226; a5_x = 0.069; 
a6_x = -0.006; a7_x = 0.056; a8_x = 0.486;

C_x = 1.65;


a1_y = -22.1; a2_y = 1011; a3_y = 1078; a4_y = 1.82; a5_y = 0.208; 
a6_y = 0.0;  a7_y = -0.354; a8_y = 0.707;

C_y=1.30;
%%

Fz_f = 2*(b * m * 9.81) / (1000 * 2 * (a+b)); %force in K-newtons
Fz_r = 2*(a * m * 9.81) / (1000 * 2 * (a+b)); %force in K-newtons
...
for k = 1:1:2
 ...
 BCD_y = a3_y * sin(a4_y * atan(a5_y * Fz(k)));
 B_y = BCD_y / (C_y * D_y);
 E_y = (a6_y * (Fz(k)^2)) + a7_y * Fz(k) + a8_y;
 
 
 Fl(i,j) = D_x * sin(C_x * atan(B_x * (alpha*pi/180)));
 Fc(i,j) = D_y * sin(C_y * atan(B_y * (alpha*pi/180)));
 end
